...
contract SupplyChain is FarmerRole, ConsumerRole, RetailerRole, DistributorRole {
  ...
  function processItem(uint _upc) public 
      harvested(_upc) //modifier for checking if item has been harvested
      verifyCaller(items[_upc].ownerID) // verifying caller ID, only owner of item can verify
  {
  ...
  function packItem(uint _upc) public 
      processed(_upc) //modifier to check if processed
    ...
    checkValue(_upc) // Check value modifier
{
  ...
  function shipItem(uint _upc) public 
    sold(_upc) //is item sold? modifier
    verifyCaller(items[_upc].distributorID) //verify caller is distributor
  {
    // Update the appropriate fields
